...
a difference well inside the $plus.minus 0.022$ lung seed spread, so not a genuine improvement)
...
integrated into the shared pipeline. Its forward pass and training loop were
verified to run correctly inside the data-loading and evaluation framework, so the
architecture is ready to train under the same protocol as the other deep models.
A full fine-tuning and multi-seed evaluation of the AST lies beyond the scope of
the present comparison and is reserved for future work. The integration itself is
complete and the code is available in the repository.
