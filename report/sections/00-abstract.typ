...
tuned EfficientNet-B0 reaches 0.898 ± 0.008 — comparable, though not ahead of it.
On lung sounds all methods occupy one performance tier (ICBHI ≈ 0.54–0.56),
EfficientNet-B0 marginally best at 0.555 ± 0.016. The cross-modal analysis reveals
asymmetric transfer: lung-pretrained features carry over strongly to heart, but
heart→lung transfer is near-neutral; a joint multi-task model roughly preserves
both. An Audio Spectrogram Transformer was integrated and verified but not fully
fine-tuned within the time budget. The study is deliberately scoped as honest,
leakage-free baselines, with a limitations section delineating what the results do
...
достигает 0.898 ± 0.008 — сопоставимо, но классическая модель остаётся численно
лучшей. На дыхательных шумах все методы находятся в одном диапазоне
(ICBHI ≈ 0.54–0.56), лучший — 0.555 ± 0.016 (EfficientNet-B0). Межмодальный анализ
