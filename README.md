...
> [!NOTE]
> **Что с данными уже в репозитории.** Для прозрачности проверки метаданные хранятся прямо здесь:
> `data/processed/manifest.csv` (индекс всех записей: путь, пациент/запись, метка, модальность,
> база A–E), `data/processed/lung_cycles.csv` (аннотации дыхательных циклов ICBHI) и
> `results/splits/*.csv` (точный состав train/test — доказательство отсутствия утечки). Так
> проверяющий видит, на чём именно учили и тестировали. Само **аудио** в репозиторий не кладётся
> (объём ~2.4 ГБ + ICBHI не лицензирован для перераспространения) и восстанавливается скриптами выше.
...
data/processed/          # манифест записей + аннотации циклов ICBHI (метаданные, не аудио)
