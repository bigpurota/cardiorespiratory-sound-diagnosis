"""Run the classical (features + classifiers) experiments per"""
import argparse
import os
import sys
...
def _load_cache(modality):
    """Load the feature cache for ``modality`` and return the"""
...
def _write_metrics_csv(modality, rows):
    """Write the per-modality metrics CSV with the full metric"""
...
def _rebuild_unified(modality, rows):
    """Idempotently merge this modality's classical rows into"""
...
def _rebuild_volumetrics(modality, rows, cache_path):
    """Merge this modality's per-run volumetrics into"""
...
def run_modality(modality):
    """Run all experiments for ``modality`` and write the three"""
